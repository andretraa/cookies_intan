...
@section('title', 'Kelola Katalog & Foto Menu')
...
@section('content')
    ...
    <div class="page-header">
        ...
        <a href="{{ route('admin.catalog.create') }}" class="btn-add">
    ...
    <div class="filter-bar">
        <div class="filter-pills">
            <a href="{{ route('admin.catalog.index', ['search' => request('search')]) }}" 
               class="filter-pill {{ !request('category') || request('category') === 'all' ? 'active' : '' }}">
                Semua ({{ $counts['total'] }})
            </a>
            @foreach($categories as $catKey => $catLabel)
                <a href="{{ route('admin.catalog.index', ['category' => $catKey, 'search' => request('search')]) }}" 
                   class="filter-pill {{ request('category') === $catKey ? 'active' : '' }}">
                    {{ $catLabel }} ({{ $counts[$catKey] ?? 0 }})
                </a>
            @endforeach
        </div>

        <form action="{{ route('admin.catalog.index') }}" method="GET" class="search-box">
            @if(request('category'))
                <input type="hidden" name="category" value="{{ request('category') }}">
            @endif
            <i class="fa-solid fa-magnifying-glass search-icon"></i>
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama menu..." onchange="this.form.submit()">
        </form>
    </div>
                    ...
                    <tbody>
                        @foreach($products as $product)
                            <tr>
                                ...
                                <td>
                                    <div class="product-meta">
                                        <div class="thumb-wrap">
                                            <img src="{{ $product->image_url }}" alt="{{ $product->name }}" class="thumb-img" loading="lazy">
                                        </div>
                                        <div>
                                            <div class="product-title-text">{{ $product->name }}</div>
                                            <div class="product-desc-snippet" title="{{ $product->description }}">{{ $product->description ?: 'Tidak ada deskripsi' }}</div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <!-- Category -->
                                <td>
                                    <span class="badge-category">{{ $categories[$product->category] ?? ucfirst($product->category) }}</span>
                                </td>
                                ...
                                <td>
                                    @if($product->badge)
                                        <span class="badge-promo">{{ $product->badge }}</span>
                                    @else
                                ...
                                <!-- Status Toggle -->
                                <td>
                                    <form action="{{ route('admin.catalog.toggle', $product) }}" method="POST" style="display: inline;">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" class="status-btn {{ $product->is_active ? 'active' : 'inactive' }}" title="Klik untuk mengubah status">
                                ...
                                <td>
                                    <div class="actions-wrap" style="justify-content: flex-end;">
                                        <a href="{{ route('admin.catalog.edit', $product) }}" class="btn-action-edit" title="Ubah foto katalog & data menu">
                                            <i class="fa-solid fa-pen-to-square"></i> Ubah Foto / Data
                                        </a>

                                        <form action="{{ route('admin.catalog.destroy', $product) }}" method="POST" style="display: inline;" 
                                              onsubmit="return confirm('Apakah Anda yakin ingin menghapus menu \'{{ $product->name }}\'? Foto dan data menu ini akan dihapus permanen.');">
                                            @csrf
                                            @method('DELETE')
